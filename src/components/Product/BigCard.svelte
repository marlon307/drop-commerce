...
<a
  ...
  class="group relative block aspect-square h-full w-full overflow-hidden rounded-lg border border-slate-200 bg-white transition-colors dark:border-neutral-800 dark:bg-black"
>
  <picture class="relative flex h-full">
    ...
    <div class="absolute bottom-8 left-0 px-4 md:bottom-[35%] md:left-12">
      {#if oldPrice}
        <span
          class="mr-4 mb-1 ml-auto block w-min rounded-full border border-slate-200 bg-white/70 px-2 py-1 text-xs text-blue-400 line-through backdrop-blur-md md:text-sm dark:border-neutral-800 dark:bg-neutral-950/70"
        >
      ...
      {/if}
      <div
        class="font-semibol flex items-center gap-2 rounded-full border border-slate-200 bg-white/70 p-1 font-medium text-slate-900 backdrop-blur-md dark:border-neutral-800 dark:bg-neutral-950/70 dark:text-white"
      >
        <span
