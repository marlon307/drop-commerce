...
{#if show}
  ...
  >
    <div class="flex justify-between items-center mb-4">
      ...
      <button
        class="p-2 rounded-lg border border-neutral-700 float-right"
        on:click={() => (show = !show)}
        data-close
      >
        <svg
    ...
    <ul class="flex-1 block overflow-auto">
      <li class="flex gap-4 border-b border-neutral-700 py-6">
        <a href="/" class="flex gap-4 justify-between">
          <figure
            class="w-20 h-20 rounded-lg border border-neutral-700 hover:bg-neutral-800 bg-neutral-900 overflow-hidden"
          >
            <img
              src="https://images.yampi.me/assets/stores/3072/uploads/images/cabo-carredaor-iphone-651f3697833b0-medium.png"
              alt="sdasdas"
              loading="lazy"
          ...
          <div class="flex-1">
            <span class="text-neutral-100 text-xl text-left line-clamp-2">
              Cabo carredaor iPhone
            </span>
            <div class="text-neutral-400 font-light">Brnaco / G</div>
          </div>
        </a>
        <div class="text-neutral-100 flex flex-col gap-5 items-start">
          <span>
            {(32).toLocaleString("pt-BR", {
              style: "currency",
              currency: "BRL",
            })}
          </span>
